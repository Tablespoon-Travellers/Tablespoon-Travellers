var spinner = (function () {
  ...
  var t = (t) => (2 * Math.PI) / t;
  var e = (t) => {
    const { originX: e, originY: n, spinnerRadius: i, pinRadius: r } = t;
    return `<circle cx=${e} cy=${n} r=${
      i - 2 * r
    } fill=transparent stroke-width=${4 * r} stroke=var(--shade) />`;
  };
  var n = (t) => {
    const { viewBoxSize: e, originX: n, originY: i, spinnerRadius: r } = t;
    return `<mask id="mask"><rect width="${e}" height="${e}" fill="#000"/><circle cx=${n} cy=${i} r=${r} fill="#fff"/></mask>`;
  };
  var i = (t) =>
    `<path d="M${
      t - 4
    } 3 h12 l-6 10z" fill=var(--shade) stroke-linejoin="round" stroke-width=2 stroke=var(--text) />`;
  var r = (t, e, n, i) => [n * Math.cos(i) + t, n * Math.sin(i) + e];
  var s = (e, n) => {
    const { originX: i, originY: s, spinnerRadius: a, pinRadius: o } = n;
    let l = "",
      h = Math.min(e, 60);
    const c = t(h);
    for (let t = 0; t < h; t++) {
  ...
  var o = (e, n) => {
    ...
    return e.map((t, e) => {
      ...
      return {
        ...
        text: `<text fill=var(--text)><textPath ${
          n ? "" : "text-anchor=middle startOffset=50%"
        } href=#t${e}>${t}</textPath></text>`,
      };
    });
  };
  var l = (t) =>
  ...
  const c = {
    ...
    patterns: [
      "m1 2a1 1 0 102 0a1 1 0 10-2 0",
      "m0 0h1l3 3v1h-1l-3-3z M3 0h1v1z M0 3v1h1z",
  ...
  var d = (r, a = c) => {
    a = { ...c, ...a };
    const d = t(r.length);
    a.fontSize = r.length > 100 ? d * a.spinnerRadius * 0.9 : 12;
    const p = o(r, a),
      u = l(a.patterns),
      m = n(a),
      g = h(r, a),
    ...
    return `\n            <div id="spinner-inner">\n            <svg viewBox="0 0 ${
      a.viewBoxSize
    } ${a.viewBoxSize}" height=100% width=100% font-size="${
      a.fontSize
    }">\n                <defs>\n                ${u}\n                ${m}\n                ${p
      .map((t) => t.path)
      .join(
      ...
      a.originY
    } r=5 fill=var(--base) stroke=var(--shade) stroke-width=2></circle>\n            </svg>\n            </div>\n            <div id="ticker">\n                <svg viewBox="0 0 ${
      a.viewBoxSize
    } ${
      a.viewBoxSize
  ...
  (v.prototype.setup = function () {
    ...
    (this.bandpass = this.audioContext.createBiquadFilter()),
      ...
      (this.oscillators = []),
      [2, 3, 4.16, 5.43, 6.79, 8.21].forEach((t) => {
    ...
    (v.prototype.trigger = function () {
      this.setup(),
        ...
        this.oscillators.forEach((t) => {
          t.start(this.audioContext.currentTime + 0.01),
            t.stop(this.audioContext.currentTime + 0.07);
        });
    });
  var $ = () => {
    new v(f).trigger();
  ...
  const I = 2 * Math.PI,
    ...
    P = () => {
      ...
      (B.sectorAngle = I / e),
        (B.pinRadiansWithTickerOffset = A(e, B.maxNumberOfPins)),
        (M.spinnerInner = document.getElementById("spinner-inner")),
        (M.ticker = document.getElementById("ticker"));
    },
    S = () =>
      window.countries.filter((t) => t.name.trim())
        .map((t) => t.name.trim()),
    A = (t, e) => {
      let n = [],
        i = Math.min(t, e),
    ...
    V = (t, e) => {
      (M.spinnerInner.style.transitionTimingFunction = `cubic-bezier(${t.join(
        ","
      )})`),
        (M.spinnerInner.style.transitionProperty = "transform"),
        (M.spinnerInner.style.transitionDuration = e / 1e3 + "s");
    },
    E = { startTimestamp: null, lastTimestamp: null, down: !0 },
    O = (t) => {
      E.startTimestamp || (E.startTimestamp = t),
        E.lastTimestamp || (E.lastTimestamp = t);
      const e = t - E.startTimestamp,
        n = y(M.spinnerInner);
      q(n)
        ? ((M.ticker.style.transform = "translateY(-3px)"),
          E.down && B.sound && $(),
          (E.down = !1))
        : t - E.lastTimestamp > 20 &&
          ((M.ticker.style.transform = "translateY(0)"), (E.down = !0)),
        e < B.spinDuration
          ? window.requestAnimationFrame(O)
          : (E.startTimestamp = null);
    },
    q = (t) =>
      B.pinRadiansWithTickerOffset.some((e) => u(t, e - 0.05, e + 0.05)),
    L = (t) => {
      M.tada.classList.add("highlight"),
        (M.tadaText.textContent = t),
        (M.removeName.textContent = t),
        (M.removeButton.disabled = !1),
        setTimeout(() => {
    ...
    Y = () => {
      (M.tadaText.textContent = ""),
        (M.removeButton.disabled = !0),
        (M.removeName.textContent = "winner");
    };
  let j = "";
  (() => {
    const t = x();
    console.log(t),
      t &&
        t.values &&
        ((M.list.value = Array.isArray(t.values)
          ? t.values.join("\n")
          : [t.values]),
